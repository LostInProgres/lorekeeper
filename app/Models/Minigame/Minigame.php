...
class Minigame extends Model
{
     ...
     */
    protected $fillable = [
        'name', 'sort', 'has_image', 'description', 'parsed_description', 'is_visible', 'minigame_type', 'data', 'limit', 'limit_period', 'output', 'variable_data', 'currency_id', 'fee', 'flavor_data',
    ];
    ...
    {
        $class = 'App\Services\Minigame\\' . str_replace(' ', '', ucwords(str_replace('_', ' ', $this->minigame_type))) . 'Service';
        return (new $class());
    }
    ...
    {
        return config('lorekeeper.minigame_types.' . $this->minigame_type);
    }
    ...
    {
        return 'images/data/minigames/images';
    }
    ...
    /**
     * Get the currency for the minigame fee
     */
    ...
    public function logCount($user)
    {
        if (isset($this->limit)) {
            ...
            switch ($this->limit_period) {
                case null:
                    return MinigameLog::played($this->id, $user->id)->count();
                    break;
                case 'Hour':
                    return MinigameLog::played($this->id, $user->id)->where('created_at', '>=', now()->startOfHour())->count();
                    break;
                case 'Day':
                    return MinigameLog::played($this->id, $user->id)->where('created_at', '>=', now()->startOfDay())->count();
                    break;
                case 'Week':
                    return MinigameLog::played($this->id, $user->id)->where('created_at', '>=', now()->startOfWeek())->count();
                    break;
                case 'Month':
                    return MinigameLog::played($this->id, $user->id)->where('created_at', '>=', now()->startOfMonth())->count();
                    break;
                case 'Year':
                    return MinigameLog::played($this->id, $user->id)->where('created_at', '>=', now()->startOfYear())->count();
                    break;
            }
    ...
    /**
     * Scope a query to only include visible minigames.
     *
