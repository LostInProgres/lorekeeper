...
class GeneralService extends Service
{
    ...
    public function handleChecks($arcade, $user)
    {
        ...
        try {
            ...
            if ($arcade->limit) {
                if (! $arcade->checkLimit($user)) {
                    throw new \Exception("You have already completed this game the maximum number of times.");
                }
            }


            if (! $arcade->checkCurrencyCap($user)) {
                    throw new \Exception("You have reached the daily currency cap.");
                }
            ...
            if ($arcade->currency && isset($arcade->fee)) {
                if (! (new CurrencyManager())->debitCurrency($user, null, $gameName . ' Fee', $gameName . ' fee ( ' . $arcade->displayName . ')', $arcade->currency, $arcade->fee)) {
    ...
    public function grantRewards($arcade, $user)
    {
        ...
        try {
            ...
            if (Settings::get('arcade_global_cap') != 0 || isset($arcade->currency_cap)) {
                if (! empty($arcade->rewardItems['currencies'])) {
                    $currrewards = null;
                    foreach ($arcade->rewardItems['currencies'] as $reward) {
                        $currrewards = $currrewards + $reward['quantity'];
                    }

                    ArcadeLog::played($arcade->id, $user->id)->orderBy('created_at', 'DESC')->first()->update(['currency_earned' => $currrewards]);
                }
            }
