    ...
    */
    ...
    public function pullNumber($categoryId)
    {
        ...
        if (Config::get('lorekeeper.settings.character_pull_number') == 'all') {
            ...
            $character = Character::myo(0)
                ->where('character_category_id', $categoryId)
                ->orderBy('number', 'DESC')
                ->first();
            if ($character) {
                $number = ltrim($character->number, 0);
    ...
    private function handleCharacter($data, $isMyo = false)
    {
        try {
            ...
            $characterData = Arr::only($data, ['character_category_id', 'rarity_id', 'user_id', 'number', 'slug', 'description', 'sale_value', 'transferrable_at', 'is_visible']);
    ...
    public function updateCharacterStats($data, $character, $user)
    {
        ...
        try {
            if (
                ...
                Character::where('slug', $data['slug'])
    ...
    public function processTransfer($data, $user)
    {
        ...
        try {
            $transfer = CharacterTransfer::where('id', $data['transfer_id'])
                ->active()
    ...
    public function moveCharacter($character, $recipient, $data, $cooldown = -1, $logType = null)
    {
        ...
        $this->createLog(is_object($sender) ? $sender->id : null, is_object($sender) ? null : $sender, $recipient && is_object($recipient) ? $recipient->id : null, $recipient && is_object($recipient) ? $recipient->url : ($recipient ?: null), $character->id, $logType ? $logType : ($character->is_myo_slot ? 'MYO Slot Transferred' : 'Character Transferred'), $data, 'user');
    }
    ...
    public function createDesignUpdateRequest($character, $user)
    {
        ...
        try {
            ...
            if (
                CharacterDesignUpdate::where('character_id', $character->id)
                    ->active()
                    ->exists()
            ) {
    ...
    public function saveRequestImage($data, $request, $isAdmin = false)
    {
        ...
        try {
            ...
            if (!$isAdmin || ($isAdmin && isset($data['modify_thumbnail']))) {
                $imageData = [];
                if (isset($data['use_cropper'])) {
                    $imageData = Arr::only($data, ['use_cropper', 'x0', 'x1', 'y0', 'y1']);
                    $imageData['use_cropper'] = isset($data['use_cropper']);
                }
    ...
    public function saveRequestFeatures($data, $request)
    {
        ...
        try {
            ...
            $features = Feature::whereIn('id', $data['feature_id'])
                ->with('rarity')
                ->get()
                ->keyBy('id');
    ...
    public function approveRequest($data, $request, $user)
    {
        ...
        try {
            ...
            if (isset($requestData['user']['currencies']) && $requestData['user']['currencies']) {
                foreach ($requestData['user']['currencies'] as $currencyId => $quantity) {
                    $currency = Currency::find($currencyId);
                    if (!$currencyManager->createLog($request->user_id, 'User', null, null, $request->character->is_myo_slot ? 'MYO Design Approved' : 'Character Design Updated', 'Used in ' . ($request->character->is_myo_slot ? 'MYO design approval' : 'character design update') . ' (<a href="' . $request->url . '">#' . $request->id . '</a>)', $currencyId, $quantity)) {
            ...
            if (isset($requestData['character']['currencies']) && $requestData['character']['currencies']) {
                foreach ($requestData['character']['currencies'] as $currencyId => $quantity) {
                    $currency = Currency::find($currencyId);
                    if (!$currencyManager->createLog($request->character_id, 'Character', null, null, $request->character->is_myo_slot ? 'MYO Design Approved' : 'Character Design Updated', 'Used in ' . ($request->character->is_myo_slot ? 'MYO design approval' : 'character design update') . ' (<a href="' . $request->url . '">#' . $request->id . '</a>)', $currencyId, $quantity)) {
     ...
     * @return  bool
     */
    public function grantCharacterFeatures($data, $character, $staff, $type)
    {
        DB::beginTransaction();
        ...
        try {
            ...
            if (!$data['feature_ids']) {
                throw new \Exception('No traits selected');
            }
            $features = Feature::find($data['feature_ids']);
            if ($type == 'Remove') {
                foreach ($features as $feature) {
                    $this->debitFeature($character, $staff, $feature);
                }
            } elseif ($type == 'Grant') {
                foreach ($features as $feature) {
                    $this->creditFeature($staff, $character, $feature);
                }
            }
    ...
    public function creditFeature($sender, $recipient,$feature)
    {
        ...
        try {
            $recipient_stack = UnlockedFeature::where([['character_id', '=', $recipient->id], ['feature_id', '=', $feature->id]])->first();
