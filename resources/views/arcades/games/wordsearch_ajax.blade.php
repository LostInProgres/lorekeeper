...
<div class="puzzleWrap text-center">
    ...
    <div class="form-group">
        ...
        {!! Form::hidden('word_count', $word_count) !!}
    ...
    */
    ...
    (function(document, $, wordfind) {
        ...
        var WordFindGame = function() {
            ...
            var drawWords = function(el, words) {
                ...
                for (var i = 0, len = words.length; i < len; i++) {
                    var word = words[i];
                    output += '<div class="col-md-2 word ' + word + '">' + word + '</div>';
                }
                output += '</div>';
            ...
            var startTurn = function() {
                $(this).addClass('selected text-info');
                startSquare = this;
            ...
            var select = function(target) {
                ...
                if (newOrientation) {
                    ...
                    if (lastSquare !== startSquare) {
                        $(lastSquare).removeClass('selected text-info');
            ...
            var playTurn = function(square) {
                ...
                for (var i = 0, len = wordList.length; i < len; i++) {
                    if (wordList[i].indexOf(curWord + $(square).text()) === 0) {
                        $(square).addClass('selected text-info');
            ...
            var endTurn = function() {
                ...
                for (var i = 0, len = wordList.length; i < len; i++) {
                    ...
                    if (wordList.length === 0) {
                        $('.puzzleSquare').addClass('complete text-success');
                    }
                }

                // reset the turn
                $('.selected').removeClass('selected text-info');
                startSquare = null;
                selectedSquares = [];
                curWord = '';
                curOrientation = null;
    ...
    $(function() {
        ...
        $('form').submit(function(e) {
            ...
            if (mini && found.length < mini) {
                alert('Not enough words found to submit for rewards. You have found ' + found.length + ' / ' + mini + '.');
                return false;
            } else if (found.length == 0) {
                alert('Cannot submit a blank word search.');
                return false;
            }

            if ('<?php echo $award_per_word; ?>' == 0 && found.length != '<?php echo $word_count; ?>') {
                alert('You cannot submit a word search that is incomplete.');
                return false;
            }
            this.submit();
