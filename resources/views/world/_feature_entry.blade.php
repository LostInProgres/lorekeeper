<div class="row world-entry">
    ...
    <div class="{{ $feature->has_image ? 'col-md-9' : 'col-12' }}">
        <h3>{!! $feature->displayName !!} <a href="{{ $feature->searchUrl }}" class="world-entry-search text-muted"><i class="fas fa-search"></i></a>
         @if ($feature->is_default)
                <i class="fas fa-unlock" data-toggle="tooltip"
                    title="Availiable to all characters"></i>
            @else
            <i class="fas fa-lock" data-toggle="tooltip"
                    title="Must be unlocked to use."></i>
                    @endif</h3>
